class RecipeItemsController < ApplicationController
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to recipe_items_path, status: :see_other, notice: "Recipe item was successfully destroyed." }
  ...
  private
    ...
    def recipe_item_params
      params.expect(recipe_item: [ :recipe_id, :name, :amount ])
    end
end
