class IngredientAggregatorService
  # Main method to aggregate ingredients from recipe items
  def self.aggregate(recipe_items)
    new(recipe_items).aggregate
  end

  def initialize(recipe_items)
    @recipe_items = recipe_items
  end

  def aggregate
    # Process and aggregate ingredients
    aggregated_ingredients = aggregate_ingredients(@recipe_items)
    finalize_aggregation(aggregated_ingredients)
  end
